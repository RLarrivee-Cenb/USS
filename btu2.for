...
c
C
	WRITE (6,*) ' TREND CHART FOR BTUS/TON- STEP 2 AGGLOM'
333	WRITE (6,*) ' '
	WRITE (6,*) ' '
...
C
	read (key(3:4),'(i2)') ll
	ldat=key(5:6)//'-'//mon(ll)//'-20'//key(1:2)
...
663	UNLOCK (50)
	if (iip .eq. 0) then
		if (for.mbtu(1,3) .eq. 0 .and. for.mbtu(2,3) .eq. 0 .and.
	1		for.mbtu(3,3) .eq. 0) go to 521
	end if
c
C
	XVAL(I2,1)=FOR.MBTU(1,3)
	XVAL(I2,2)=FOR.MBTU(2,3)
	XVAL(I2,3)=FOR.MBTU(3,3)
C
	IF (I2-1 .LT. 1) GO TO 92
	DTM(1,I2-1)=NDAT(1)
	DTM(2,I2-1)=NDAT(2)
92	I2=I2-1
C
...
c
	if (iip .eq. 0) then
		if (for.mbtu(1,2) .eq. 0 .and. for.mbtu(2,2) .eq. 0 .and.
	1		for.mbtu(3,2) .eq. 0) go to 522
	end if
c
c
	XVAL(I2,1)=FOR.MBTU(1,2)
	XVAL(I2,2)=FOR.MBTU(2,2)
	XVAL(I2,3)=FOR.MBTU(3,2)
C
	IF (I2-1 .LT. 1) GO TO 93
	DTM(1,I2-1)=NDAT(1)
	DTM(2,I2-1)=NDAT(2)
93	I2=I2-1
C
...
c
	if (iip .eq. 0) then
		if (for.mbtu(1,1) .eq. 0 .and. for.mbtu(2,1) .eq. 0 .and.
	1		for.mbtu(3,1) .eq. 0) go to 523
	end if
c
c
	XVAL(I2,1)=FOR.MBTU(1,1)
	XVAL(I2,2)=FOR.MBTU(2,1)
	XVAL(I2,3)=FOR.MBTU(3,1)
C
	IF (I2-1 .LT. 1) GO TO 94
	DTM(1,I2-1)=NDAT(1)
	DTM(2,I2-1)=NDAT(2)
94	I2=I2-1
C
...
C
C
	LABEL='    STEPS 1 & 2 TOTAL MBTUS/TON by shift'
	X(1)=.16
	Y(1)=.83
...
c
	JJE=ABC
	WRITE (RSIDE(9:11),'(I3)') JJE
	RSIDE='S1&2 B.P.'//RSIDE(9:11)
C
C     
	Y(1)=Y(1)+.002
	X(2)=.02
...
C
	...
	Y(1)=(abc-YAXB)*.05625*YJ+.42
	Y(2)=Y(1)
CCCCCCCC	CALL GKS$POLYLINE (N21,X,Y)
C
	JJE=ABC
	WRITE (RSIDE(9:11),'(I3)') JJE
	RSIDE='S2 B.P. '//RSIDE(9:11)
C
C
	Y(1)=Y(1)+.002
	X(2)=.02
CCCCCCCC	CALL GKS$TEXT (X(2),Y(1),RSIDE)
C
C
C
...
99	CONTINUE
	END
	subroutine clean_f_zero (i)
		integer*4 i
		if ((i .and. z'0000ff80') .eq. 0) i=0
	return
	end
