...
C	DIGITAL POINT NUMBERS FROM THE COMMON SDXR_03
C
C	LINK:  CXSCANNERAG3,DIGCOMAG3/OPT,WESAPI_LIB:WESAPI/OPT
C
C
	PROGRAM CXSCANNERAG3
C
C
	INCLUDE '($IODEF)'
	INCLUDE '($SSDEF)'
...
C
C
	COMMON  /CONTACTSAG3/CXSTS,/CXALARMAG3/CXALRM  !common that this prog. 
C
C
...
C
C	  READ IN CX POINT NOS. FROM CXIAG3.SRC
C
	OPEN (UNIT=50,FILE='USER_D:[RJM]CXIAG3.SRC',STATUS='OLD',
	1	SHARED,READONLY)
C
	JJ=0
	KK=1
