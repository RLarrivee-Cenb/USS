...
C
C	LINK:  CXSCANNERCRS,DIGCOMCRS/OPT,WESAPI_LIB:WESAPI/OPT
C
C	09-20-04 ADDED POINT 2405 & MODIFIED PT. 2381 FOR WJB
C
	PROGRAM CXSCANNERCRS
C
C
	INCLUDE '($SSDEF)'
	INCLUDE '($IODEF)'
...
C
	COMMON  /CONTACTSCRS/CXSTS,/CXALARMCRS/CXALRM  !common that this prog. 
C                                                      !writes to 
C
...
C
	OPEN (UNIT=50,FILE='USER_D:[RJM]CXICRS.SRC',STATUS='OLD',
	1	SHARED,READONLY)
C
	JJ=0
	KK=1
...
C
C
C		check to see if common "cxi" is running
C
221	ISTATUS=SYS$ASCEFC(%VAL(65),CLUSTER,,)
C
	IF (.NOT. ISTATUS) THEN
C
		IF (PASS .GT.0) GO TO 200	
		OPEN (UNIT=1,FILE='LTA016:',STATUS='UNKNOWN',CARRIAGE
	1	CONTROL='LIST')
C
		WRITE (1,43)
43		FORMAT (' ','DIGITAL INPUT COMMON "CXI" IS STOPPED')
C
		CLOSE (1)
C
		PASS=1
...
C
C
C	   ----	SECTION TO SEE IF 'RELCXPROG' HAS TO BE TURNED ON ----
C
C
CCCCCC	IF (CREP) 200,200,700
C
CCCCCC700	STATUS=SYS$WAKE (,%DESCR(RELPR))
C
CCCCCC	IF (.NOT. STATUS) CALL LIB$SIGNAL (%VAL(STATUS))
C
C
C
C
