...
c
61      OPEN (UNIT=10,FILE='user_d:[flux]SLURRY22.DAT',
...
C
774     MMSIL=0
CCCCCCCC        IF ((DAT.SIL(INDEX) .LT. 3.43)
CCCCC   1       .OR.(DAT.SIL(INDEX) .GT. 4.00)) MMSIL=1
C
        IF (dat.AL(INDEX)+dat.SIL(INDEX) .LE. 0) GO TO 223
        BA=(dat.CA(INDEX)+dat.MG(INDEX))/(dat.AL(INDEX)+dat.SIL(INDEX))
C
        MMBA=0
        IF ((BA .LT. 1.02) .OR. (BA.GT. 1.38)) MMBA=1
C
C
        IF (BYPASS .GE. 1) GO TO 223
        BYPASS=1
C
C       IF (MMSIL .GT. 0 .AND. MMBA .GT. 0) THEN
C
C               CALL FDV$cDISP ('BOTHOUT')
C               CALL FDV$GET (AA,IT,'REPLY')
C               call fdv$dISP ('flux')
C       CALL FDV$PUT (ADATE(1:2)//ADATE(4:6)//ADATE(10:11),'DATE')
C       CALL FDV$PUT (Adate(13:14),'TIME')
C       write (aa,'(f5.2)',err=3773) ba
C       call fdv$put (aa,'base')
c
C3773   write (aa,'(f5.4)',err=575) R
C       CALL FDV$PUT (AA,'RATIO')
C               GO TO 575
C
C       ELSE IF (MMSIL .GT. 0 .AND. MMBA .LE. 0) THEN
C
C               CALL FDV$cDISP ('BOTHOUT')
C               CALL FDV$GET (AA,IT,'REPLY')
C               call fdv$dISP ('flux')
C       CALL FDV$PUT (ADATE(1:2)//ADATE(4:6)//ADATE(10:11),'DATE')
C       CALL FDV$PUT (Adate(13:14),'TIME')
C       write (aa,'(f5.2)',err=4773) ba
C       call fdv$put (aa,'base')
c
C4773   write (aa,'(f5.4)',err=575) R
C       CALL FDV$PUT (AA,'RATIO')
C               GO TO 575
C
        IF (MMSIL .LE. 0 .AND. MMBA .GT. 0) THEN
C
                CALL FDV$cDISP ('BAOUT')
...
c
        ...
        WRITE (28,2828,ERR=556) '2',ADS(1:11),ADS(13:14),STM
...
C
C
333     OPEN (UNIT=10,FILE='user_d:[flux]SLURRY22.DAT',
...
C
        IF (MM .EQ. 52) THEN
                ...
                IF (K4 .GE. 20) THEN
                CALL SYS$ASCTIM (,QED,,)
                OPEN (86,FILE='USER_D:[FLUX]ERRORS.LIS',STATUS='UNKNOWN',
        1               ACCESS='APPEND')
...
C
C
        OPEN (UNIT=44,FILE='DISK0:[FLUX]CAL2.DAT',
