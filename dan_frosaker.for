...
C
	...
	1	,MCBCR(150),OIBCR(150),MBCR(200),YBCR(200)
...
C
	...
	1	CRBMI*23/'USER_D:[METREP.BMCR]BMI'/ 
	1	,FNDSTRNG*80,FNM(5)*80,TOT(51)*24,D2*4,CRFNM*80
...
C
	...
	1       'MINE IND. MAG FE        ','ROD MILL DAV. TUBE SIO2 ',
	1       '                        ','                        ',
	1       '                        ','                        ',
	1       '                        '/
C
...
C	WRITE (6,*) ' '
C	WRITE (6,*) ' '
C	WRITE (6,*) ' CONCENTRATOR COMBINED MET REPORT'
C	WRITE (6,*) ' '
C	WRITE (6,*) ' '
C
C                                                          
	CALL TIME(TIM)
C
	OPEN (UNIT=29,FILE='MET_P:DAN_FROSAKER.DAT',
	1		STATUS='UNKNOWN',ACCESS='APPEND')
C
	CALL SYS$BINTIM (AINTV,INTV) !convert 24 hr interval to binary
C
	CALL SYS$BINTIM ('01-JAN-2005',BDS)
C	CALL SYS$GETTIM (BDS)      !present time
C
	PASS=0
C
3110	DO 49 JL=1,2
...
C
C
	IF (FNM(1)(31:34) .NE. '2004') GO TO 99
C
C
C
C	WRITE (6,98)
C98	FORMAT (/'0',' TYPE <RETURN> TO CONTINUE 
C	1 SEARCH, ''U'' TO USE IT, "A" TO STOP,'/
C	1 ' OR ENTER DESIRED DATE (ex: DD-MMM-YYYY)')
C
C	READ (5,908)MT33,IND
C908	FORMAT (Q,A11)
	MT33=1
C
	IND(1:1)='U'
...
C
C	bring in crusher file for requested day to get MINE IND. MAG FE
C
		CRFNM=CRBMI//AYSTR(1:11)//DAT
C
	OPEN (UNIT=44,FILE=CRFNM,FORM='UNFORMATTED',STATUS='OLD'
	1	,SHARED,USEROPEN=UFO_OPEN)
C
C                            
	READ (44) MCBCR,OIBCR,MBCR,YBCR
	CLOSE (44)
...
C
C
C		ROD MILL DAVIS TUBE SIO2
C
	GR(46,1)=((T11*OIA(52))+(T21*OIB(27)))/GR(1,1)
	GR(46,2)=((T12*MA(199))+(T22*MB(90)))/GR12
	GR(46,3)=((T13*YA(199))+(T23*YB(90)))/GR13
C
C
C
C
...
C
C		OUTPUT SECTION
	WRITE (29,2930)FNM(1)(24:34),GR(1,1),GR(3,1),GR(4,1),GR(11,1)
2930	FORMAT (1H ,A11,4F12.2)
C	WRITE (29,2929) GR(1,1)
C2929	FORMAT (1H ,F12.2)
C	WRITE (29,2929) GR(3,1)
C	WRITE (29,2929) GR(4,1)
C	WRITE (29,2929) GR(11,1)
...
808	GR(I,3)=0
	...
	CALL LIB$SUB_TIMES (BDS,INTV,BDS)
	GO TO 3110	
C
C	
99	STOP
