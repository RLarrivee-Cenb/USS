...
C	FLOAT=D_FLOAT/
C	WARN=NOALIGNMENT and linked with SUBSTVCTR,BAP2/OPT,
C	WESAPI_LIB:WESAPI/OPT.
C	Must use INCLUDE 'REDFURN:KEYDEF' for keyboard definitions.
C
C	08/01/02 modified the printing routine for the long form - every
C	now and then they would miss a record on the printout.  Still need 
C	to modify furnace 1.
...
C
        COMMON /RED2_HOT/ HOT
	COMMON /RED2_PARAMETER/ RED_PAR
	COMMON /RED2_COM/ START,STOP,FIRST_PASS,DETACH,BEEP,TOGGLE
	COMMON /RED2_DISPLAY/ DISP_HEADING
C
...
C
	COMMON /LTD2_HOT/ HOT
	COMMON /LTD2_PARAMETER/ LTD_PAR
	COMMON /LTD2_COM/ START,STOP,FIRST_PASS,DETACH,BEEP,TOGGLE
	COMMON /LTD2_DISPLAY/ DISP_HEADING
C
...
!	3  => RED_NEWTEST	Reducibility new test furnace #2
!	4  => RED_STATUS    	Reducibility test status furnace #2
!	5  => RED IDLE
!	6  => RED_REPORT 	Iso reducibility report
!	7  => LTD_PARAMETER	LTD parameter table
!	8  => LTD_NEWTEST	LTD new test furnace #2
!	9  => LTD_STATUS	LTD test status furnace #2
!	10 => LTD IDLE
...
C
	...
	CHARACTER*12	PRCNAM_RED/'COLLECT_RED2'/
	CHARACTER*12	PRCNAM_LTD/'COLLECT_LTD2'/
	CHARACTER*16	FURNACE1
	CHARACTER*17	FIND_DISPLAY_RED/'REDD:COLLECT_RED2'/
	CHARACTER*21	ERROR_RED	/'REDD:COLLECT_RED2.ERR'/
	CHARACTER*17	FIND_DISPLAY_LTD/'REDD:COLLECT_LTD2'/
	CHARACTER*21	ERROR_LTD	/'REDD:COLLECT_LTD2.ERR'/
	CHARACTER*44	DATAOK
	CHARACTER*80	LINE
C
	DATA 		TIMER_ID/1/
	EXTERNAL	AST_FURN_RUN
C
	COMMON /RED2_COM/ RED_START,RED_STOP,RED_FIRST_PASS,
	1		RED_DETACH,RED_BEEP,RED_TOGGLE               
	COMMON /LTD2_COM/ LTD_START,LTD_STOP,LTD_FIRST_PASS,
	1		LTD_DETACH,LTD_BEEP,LTD_TOGGLE                
C
	DREPLY = ' '
	DATAOK = '                                            '
...
30		IF (RED_START .EQ. 1) THEN
			CALL FDV$SIGOP
			LINE = ' '
			LINE(1:47) = 'FURNACE #2 is already running '// 
	1		'a reduction test!'
		...
		ELSE IF (LTD_START .EQ. 1) THEN
			CALL FDV$SIGOP
			LINE = ' '
			LINE(1:34)='FURNACE #2 is running an LTD test!'
...
50		IF (RED_START .EQ. 1) THEN
			CALL FDV$SIGOP
			LINE = ' '
			LINE(1:39) = 'FURNACE #2 is running '//
	1		'a reduction test!'
		...
		ELSE IF (LTD_START .EQ. 1) THEN
			CALL FDV$SIGOP
			LINE = ' '
			LINE(1:42) = 'FURNACE #2 is already running '// 
	1		'an LTD test!'
...
300	CALL FDV$PUT ('Stop detached process for FURNACE #2 <Y/N>? ',
	1		'DATAOK')
	UNDERLINE = 8
...
C
	...
	1      			%VAL(TIMER_ID),)
C
10	OPEN (UNIT=1,FILE='REDFURN:RED2_PAR.DAT',
	1	STATUS='OLD',
...
C
	GOTO 30
C
20	CALL LIB$SPAWN ('CREATE/FDL=RED_PAR REDFURN:RED2_PAR.DAT',,,
	1	TRUSTED)
	GOTO 10
C
...
C	07/01/2002 Test to see if N2 is affecting test
...
C	
	...
	END IF
C
	DELETE (UNIT=1,REC=1,ERR=100)
100	WRITE (UNIT=1,REC=1)RED_PAR.STRING(1:109)
	CLOSE (UNIT=1)
C
	STATUS = SYS$CANTIM (%VAL(TIMER_ID),)
	END
...
C
	...
	INTEGER*4	STATUS,LIB$SYS_TRNLOG,SID,GP_SID
	INTEGER*4	SID_ARRAY(1)
	...
	CHARACTER*8	PT_NAME(1)/'DL429515'/	!FURN2 ok to run              
...
C
 	INTEGER*4	START,STOP,FIRST_PASS,DETACH,BEEP,TOGGLE
	INTEGER*4	N,I,BIN_DELAY,TIMER_ID,IOS,FIELDT
...
C
	...
	COMMON /RED2_HOT/ HOT
	COMMON /RED2_PARAMETER/ RED_PAR
	COMMON /RED2_DISPLAY/ DISP_HEADING	!only used in AST_DISPLAY_HOT
C
	DREPLY = ' '
	DATAOK = '                 '
	HOT.RT = '    '
...
10	OPEN (UNIT=3,FILE='REDFURN:RED2_NEWTEST.DAT',
	1	STATUS='OLD',ACCESS='KEYED',SHARED,
	1	ERR=20)
C
	GOTO 30
C
20	CALL LIB$SPAWN('CREATE/FDL=RED_NEWTEST REDFURN:RED2_NEWTEST.DAT'
	1	,,,TRUSTED)
	GOTO 10
C
...
C
	...
	OPEN (UNIT=10,FILE='REDFURN:ERROR2.LOG',
	1	STATUS='UNKNOWN',
	1	FORM='FORMATTED',
	1	ACCESS='SEQUENTIAL')
C
	...
	IF (STATUS .NE. SS$_NORMAL) THEN
		WRITE (10,100) 
100		FORMAT (' Logical WESAPI_PDIR not defined')
	END IF 
C
...
C
	...
	1	%REF(PT_NAME(I)//CHAR(0)), SID, GP_SID, 
	1	GP_BIT_NUM, EXTENDED_FLAG, INACTIVE_FLAG)
		IF (STATUS .EQ. 0) THEN
			SID_ARRAY(I) = SID
		ELSE 
			SID_ARRAY(I) = 0
...
C  Open the connection to SHC memory for WESAPI access
	...
	DO 500 I = 1,1
		...
		STATUS = SHC_GET_DIGITAL_VAL_STAT( SID_ARRAY(I), 
	1		GP_SID, GP_BIT_NUM, DIGITAL_VAL,
	1		DIGITAL_STAT)
C                WRITE (6,450) DIGITAL_VAL,SPACE_5,DESC(I)
C450		FORMAT (' ',I2,A5,A22)
500	CONTINUE
...
C	detached process COLLECT_RED2
C-------------------------------------------------------------------------------
C
	START = 1
	STOP = 0
...
C
	...
	1		,,,TRUSTED )		
		...
		CALL LIB$WAIT(6.0)	!to be able to print from x-term
...
308			FORMAT(' ------------')
	...
	1			IOSTAT=IOS)SEC
		...
		END IF
C
C	8/1/02 Added this to make sure long form doesn't miss printing
C		any lines
C
		MOD_RESULT = MOD(CUR_SEC,INTERVAL_SEC)
...
C
	ELSE IF (OPTION .EQ. '4') THEN
		OPEN (UNIT=3,FILE='REDFURN:RED2_NEWTEST.DAT',
	1 			STATUS='OLD',ACCESS='KEYED',SHARED)
...
C
	...
	1		%VAL(TIMER_ID),)
C
10	OPEN (UNIT=2,FILE='REDFURN:LTD2_PAR.DAT',
	1	STATUS='OLD',
	1	ORGANIZATION='RELATIVE',
	1	FORM='UNFORMATTED',
	1	RECORDTYPE='FIXED',
...
C
	GOTO 30
C
20	CALL LIB$SPAWN ('CREATE/FDL=LTD_PAR REDFURN:LTD2_PAR.DAT'
	1		,,,TRUSTED)
	GOTO 10
C
...
C
	...
	COMMON /LTD2_DISPLAY/ DISP_HEADING
	COMMON /LTD2_COM/ START,STOP,FIRST_PASS,DETACH,BEEP,TOGGLE
	COMMON /LTD2_TIME/ RED_ELAPSED
C
	HOT.ELAPSED = '        '
	RED_ELAPSED(13:20) = '        '
	DISP_HEADING = 0
C
	OPEN (UNIT=2,FILE='REDFURN:LTD2_PAR.DAT',
	1	STATUS='OLD',
	1	ORGANIZATION='RELATIVE',
	1	FORM='UNFORMATTED',
	1	RECORDTYPE='FIXED',
...
C-------------------------------------------------------------------------------
C
10	OPEN (UNIT=7,FILE='REDFURN:LTD2_NEWTEST.DAT',
	1	STATUS='OLD',ACCESS='KEYED',SHARED,
	1	ERR=20)
C
	GOTO 30
C
20	CALL LIB$SPAWN('CREATE/FDL=LTD_NEWTEST REDFURN:LTD2_NEWTEST.DAT'
	1	,,,TRUSTED)
	GOTO 10
C
...
C
	...
	END DO
C
	OPEN (UNIT=7,FILE='REDFURN:LTD2_NEWTEST.DAT',
	1	STATUS='OLD',ACCESS='KEYED',SHARED)
C
	N = 1
...
C
	IF (FIELDT .EQ. 103) THEN
		IF ((RED_STOP .EQ. 1).AND.(LTD_STOP .EQ. 1)) THEN
			CALL FDV$PUTL ()
			CALL FDV$PUTL ('FURNACE #2 is not running')
		ELSE
			RED_STOP = 1
			RED_START = 0
			RED_TOGGLE = 3
			...
			LTD_TOGGLE = 3
			CALL FDV$PUTL ()
			CALL FDV$PUTL ('Stopping FURNACE #2')
		END IF
	ELSE IF (FIELDT .EQ. 104) THEN
		...
		ELSE
		   	...
		   	LTD_START = 0
			CALL FDV$PUTL ()
		      CALL FDV$PUTL ('Stopping test, idling FURNACE #2')
		END IF
	END IF
C
	END
...
C	
	CHARACTER*23	RED_ELAPSED
	CHARACTER*34	MESSAGE1/'        FURNACE#2 is off          '/
	CHARACTER*34	MESSAGE2/'FURNACE#2 is on - IDLING (RED PAR)'/
	CHARACTER*34	MESSAGE3/' FURNACE#2 is on - REDUCTION TEST '/
	CHARACTER*34	MESSAGE4/'FURNACE#2 is on - IDLING (LTD PAR)'/
	CHARACTER*34	MESSAGE5/'    FURNACE#2 is on - LTD TEST    '/
C                                                  
	COMMON /RED2_COM/ RED_START,RED_STOP,RED_FIRST_PASS,
	1		RED_DETACH,RED_BEEP,RED_TOGGLE
	COMMON /LTD2_COM/ LTD_START,LTD_STOP,LTD_FIRST_PASS,
	1		LTD_DETACH,LTD_BEEP,LTD_TOGGLE
	COMMON /RED2_HOT/ RED_HOT
	COMMON /LTD2_HOT/ LTD_HOT
	COMMON /LTD2_TIME/ RED_ELAPSED
C
       	BOLDON = 1
