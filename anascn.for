...
C
	...
	1		FILE*14/'ANA:ANAMOD.DAT'/,	!ANALOG DATABASE FILE
	1		ACTIVE*15/'LOGS:ACTIVE.DAT'/,	!ACTIVE ACCUM FILE
	1		HOUR*13/'LOGS:HOUR.DAT'/,	!HOUR ACCUM FILE
	1		TIME*8/'0 0:0:10'/,	!PROGRAM RUN FREQUENCY
	1		PRCNAM*11/'LOGS:LOGSCH'/,	!LOG COLLAPSE PROGRAM
	1		ANALRM*10/'ANA:ANALRM'/		!ANALOG ALARM PROGRAM
C
	INTEGER*4	REMX,
	1		SCNCLS,
	1		LOGSCH,
...
C
	PARAMETER	(TAI=1000)	!TOTAL POINTS IN WDPF TABLE
C
	LOGICAL*4	BSTAT
C
	...
	1		ANVAL(1200),	!ANVAL TABLE STORAGE
	1		LL,		!LOW LIMIT
	1		HL,		!HIGH LIMIT
	1		REALC,		!ANAMOD REAL CONSTANT
	1		OPERT(3)/.002786,.16699,10.009/, !OPER TIME CONSTANTS
	1		CNTR,
	1		SUM,
	1		TEMP
	REAL*4		VALU,INFR
...
C
	INTEGER*4	SID(TAI)
C
	CHARACTER*6	NAMEX/'ANASCN'/
	CHARACTER	PN*8,EU*6,ED*30
	CHARACTER*14	FILE3/'ANA:ANAPHR.DAT'/
C
...
C
	...
	COMMON/ALARMTBL/ALARM		!ANALOG ALARM TABLE
C
C	INF=JIBSET(INF,15)
	INF=Z'C99E007F'
C
...
C
		OPEN	(UNIT=22,FILE='ANA:ANASCN.ERR',STATUS='UNKNOWN',
	1		 ACCESS='APPEND')
C
...
C
	...
	IF (REM .LE. 1.0) SCNCLS=1	!IF 30 SEC MARK SET FLAG FOR PULSES
C
	LOGSCH=0
...
C
2000		CONTINUE
C		QUALTY=IIBITS(AS(I),8,2)	!EXTRACT QUALITY BITS
		QUALTY=IIBITS(AS(I),9,1)	!EXTRACT QUALITY BITS
...
C
C	DO ANALOG ALARM CHECKING
C
		J=((I-1)/4)+1		!ALARM TABLE LONGWORD
		IF (MOD(I,4) .EQ. 0) THEN
			K=24
		ELSE
			K=(MOD(I,4)-1)*8	!START OF 8 BIT BLOCK
		END IF
C
		IF (HL .EQ. 0)GOTO 440	!SEE IF LOW LIMIT
		IF (ANVAL(I) .LT. HL)GOTO 440	!CHECK FOR LOW LIMIT
		ALARM(J)=JIBCLR(ALARM(J),K+1)	!RESET LOW BIT
		BIT=JIBITS(ALARM(J),K+2,1)	!EXTRACT HIGH BIT
		IF (BIT .EQ. 0) THEN	!IF NOT IN HIGH ALARM
			ALARM(J)=JIBSET(ALARM(J),K+2)	!SET HIGH BIT
			GOTO 446	!GO SET MSG BIT
		END IF
		GOTO 450
C
440		IF (LL .EQ. 0)GOTO 445	!GO CHECK RETURN TO NORMAL
		IF (ANVAL(I) .GT. LL)GOTO 445	!CHECK RETURN TO NORMAL
		BIT=JIBITS(ALARM(J),K+1,1)	!EXTRACT LOW BIT
		ALARM(J)=JIBCLR(ALARM(J),K+2)	!RESET HIGH BIT
		IF (BIT .EQ. 0) THEN	!IF NOT IN LOW ALARM
			ALARM(J)=JIBSET(ALARM(J),K+1)	!SET LOW BIT
			GOTO 446
		END IF
		GOTO 450
C
445		BIT=JIBITS(ALARM(J),K+1,2)	!EXTRACT LOW AND HIGH BITS
		IF (BIT .EQ. 0)GOTO 450		!NO WORK TO DO
		ALARM(J)=JIBCLR(ALARM(J),K+1)	!RESET LOW BIT
		ALARM(J)=JIBCLR(ALARM(J),K+2)	!RESET HIGH BIT
446		ALARM(J)=JIBSET(ALARM(J),K)	!SET MSG BIT
		ACOUNT=ACOUNT+1			!BUMP ALARM COUNTER
C
...
C
		IF (.NOT. STAT) THEN	!TURN ON ERROR
			OPEN	(UNIT=2,FILE='OPA0:',STATUS='UNKNOWN',ERR=521)
...
C
	...
	END IF
C
C	IF (ACOUNT .NE. 0)THEN	!IF WE HAVE ALARMS TURN ON ANALRM
C		STAT=SYS$CREPRC(,ANALRM,,,,,,ANALRM,%VAL(4),,,
C	1	%VAL(%LOC(PRC$M_DETACH)))
C	END IF
C
...
C
	IF (LOGSCH .EQ. 1)THEN			!IF END OF HOUR
		...
		IF (.NOT. STAT) THEN	!CANCEL ERROR
			OPEN	(UNIT=2,FILE='ANA:ANASCN.ERR',STATUS='UNKNOWN')
