...
C
	...
	1	PP1*29/'SCREEN FRACTIONS,COMPRESSIONS'/,KD*6,AAT*80             
C
...
C
	5	BAT(2)*6/'BEFORE','AFTER '/,PRES*23,TESTKEY*6,
C
	1	MNTH(12)*3/'JAN','FEB','MAR','APR','MAY','JUN','JUL',
	1'AUG','SEP','OCT','NOV','DEC'/,TDAY*11,YDAY*11,KY*6,AD*23,ascr
C
...
C
	...
	1	ACCESS='KEYED',STATUS='UNKNOWN',RECL=2500,
...
828	CONTINUE
	GO TO 5222 
C
829	WRITE (TESTKEY(3:4),'(I2)') I
	IF (TESTKEY(3:3) .EQ. ' ') TESTKEY(3:3)='0'
C
	TESTKEY(1:2)=PRES(10:11)
	TESTKEY(5:6)=PRES(1:2)
C
C
...
C
	OPEN (UNIT=65,FILE='MET_P:MMC.DAT',ACCESS='KEYED',STATUS='OLD',
	1		SHARED)
C
...
C	  check if compression is within limits.
C
cccccc	IF (NNPASS .NE. 0) GO TO 1617
C 
	IBAD=0
C
	DO 2201 LN=1,5
